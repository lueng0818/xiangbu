...
def display_piece(gua_data, pos_num):
    ...
    try:
        ...
        if image_path and os.path.exists(image_path):
            st.image(image_path, caption=f"{color}{name}", width=70) # 縮小一點以適應全盤
        else:
...
if 'current_mode' not in st.session_state: st.session_state.current_mode = "SINGLE" # SINGLE 或 FULL
...
with st.sidebar:
    ...
    if query_type == "單卦問事 (運勢/財運/感情)":
        sub_query = st.selectbox("3. 詳細事項", ["問運勢", "事業查詢", "前世格局", "健康分析", "投資/財運", "感情/關係", "離婚議題"])
        if sub_query == "投資/財運":
    ...
    if st.button("開始排盤 / 占卜"):
        if query_type == "全盤流年 (11~80歲完整排盤)":
            ...
            st.session_state.sub_query = sub_query # 記錄子選項
            # (維持原有的單卦重抽邏輯)
...
# ==============================================================================
    ...
    # 使用原本的 Tabs 架構，但根據 sub_query 動態顯示
