...
import pandas as pd
import time
from core.data import ATTRIBUTES, POSITION_MAP, get_image_path, GEOMETRY_RELATION
from core.rules import generate_random_gua, check_exemption, calculate_net_gain_from_gua, analyze_health_and_luck, is_all_same_color, check_career_pattern, check_wealth_pattern, check_consumption_at_1_or_5, check_interference
    ...
    if st.button("開始占卜：擲出五支棋"):
        ...
        if is_all_same_color(new_gua):
            ...
            if st.session_state.reroll_count == 1:
                with st.spinner('偵測到不成卦 (全黑/全紅)，正在進行第二次重抽...'): time.sleep(1); new_gua = generate_random_gua()
                if is_all_same_color(new_gua):
                    st.session_state.current_gua = new_gua; st.session_state.message = "❌ **最終警示：** 卦象連續兩次為全黑/全紅，暗示**「不會做也不會成」**。本次分析已中止。"
                    st.session_state.final_result_status = "REJECTED"
                else:
                    st.session_state.current_gua = new_gua; st.session_state.message = "🚨 第一次卦象為全黑/全紅，已重抽成功並得到有效卦象。"
                    st.session_state.final_result_status = "VALID"
            else:
                 st.session_state.message = "請刷新頁面或清除緩存後，重新開始占卜。"; st.session_state.final_result_status = "REJECTED" 
        else:
            st.session_state.current_gua = new_gua; st.session_state.reroll_count = 0; st.session_state.message = "卦象已成功生成。"
            st.session_state.final_result_status = "VALID"
